...
export function findMonthlyGroupedByDayExpenseView({ yearMonth, expenseType }) {
  return remote.postForm({
...
export function findAnnuallyGroupedByMonthExpenseView({ year, expenseType }) {
  return remote.postForm({
...
export function findMonthlyTypedAndRankeExpenseGraphView({ yearMonth }) {
  return remote.postForm({
    url: "/account-book/statistics/monthly/typed-ranked-expense",
    data: { yearMonth },
  });
}
